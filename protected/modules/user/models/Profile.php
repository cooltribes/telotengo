		...
		if (!$this->_rules) {
			...
			foreach ($model as $field) {
				...
				if ($field->field_type=='VARCHAR'||$field->field_type=='TEXT') {
					$field_rule = array($field->varname, 'length', 'max'=>$field->field_size, 'min' => $field->field_size_min, 'tooShort' => UserModule::t($field->error_message), 'tooLong' => UserModule::t($field->error_message));
					//if ($field->error_message) $field_rule['message'] = UserModule::t($field->error_message);
					array_push($rules,$field_rule);
					if($field->varname == 'cedula' || $field->varname == 'telefono'){
						array_push($rules, array($field->varname, 'match', 'pattern' => '/^[0-9]+$/', 'message' => 'Ingrese solo números'));
					}
				}
			...
			array_push($rules,array(implode(',',$required), 'required', 'message'=>'No puede estar vacío'));
