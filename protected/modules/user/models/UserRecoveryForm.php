...
class UserRecoveryForm extends CFormModel {
	...
	public function rules()
	{
		return array(
			...
			array('login_or_email', 'match', 'pattern' => '/^[A-Za-z0-9@.-\s,]+$/u','message' => 'Caracteres no permitidos'),
	...
	{
		return array(
			'login_or_email'=>'Nombre de usuario o correo electrónico',
		);
	}
	...
	public function checkexists($attribute,$params) {
		if(!$this->hasErrors())  // we only want to authenticate when no input errors
		{
			...
			if($user===null)
				if (strpos($this->login_or_email,"@")) {
					$this->addError("login_or_email",'Correo electrónico incorrecto');
				} else {
					$this->addError("login_or_email",'Nombre de usuario incorrecto');
				}
		}
	}
