...
class Empleo extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('nombre, email, fecha_nacimiento, edad, direccion, sexo, cv, lugar_nacimiento', 'required'),
			array('edad, sexo, cv', 'numerical', 'integerOnly'=>true),
			array('nombre, email', 'length', 'max'=>250),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, nombre, email, fecha_nacimiento, edad, direccion, sexo, cv', 'safe', 'on'=>'search'),
		);
	}
